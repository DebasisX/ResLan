...
def create_resume(json_file, output_file):
    ...
    if social_links:
        ...
        # computing total width including separators
        sep = " • "
        sep_w = c.stringWidth(sep, "Helvetica", 11)
        total_width = sum(c.stringWidth(lbl, "Helvetica", 11) for lbl, _ in social_links)
        total_width += sep_w * (len(social_links) - 1)
        x = (width - total_width) / 2

        for idx, (lbl, url) in enumerate(social_links):
            w = c.stringWidth(lbl, "Helvetica", 11)
            c.drawString(x, y_position, lbl)
            c.linkURL(url, (x, y_position, x + w, y_position + line_height))
            x += w
            if idx < len(social_links) - 1:
                c.drawString(x, y_position, sep)
                x += sep_w
...
# ---------- Skills ----------
    ...
    if data.get("certifications"):
        ...
        # ---------- Relevant Coursework ----------
    if data.get("relevant_coursework"):
        draw_section("Relevant Coursework")
        c.setFont("Helvetica", 11)
        
        coursework = data["relevant_coursework"]
        col_width = (width - 1.5*inch) / 2
        mid_point = left_margin + col_width + 0.5*inch
        
        # Split coursework into two columns
        split_index = (len(coursework) + 1) // 2
        col1 = coursework[:split_index]
        col2 = coursework[split_index:]
        ...
        for i in range(max_lines):
            ...
            if i < len(col1):
                bullet = "• " + col1[i]
                c.drawString(left_margin + 0.2*inch, y_position, bullet)
            
            # Column 2
            if i < len(col2):
                bullet = "• " + col2[i]
                c.drawString(mid_point + 0.2*inch, y_position, bullet)
